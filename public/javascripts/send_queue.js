$(document).ready(function() {
  ...
  var sendQueue = function () {
    ...
    if ($("#assignment_selector").val() != "Onscreen room") {
      $(".tab-e").each(function(index, el) {
        ...
        $(el).find(".list-group-item").each(function(index2, el2) {
          ...
          if (val != 0) {
            Queue.subQueue[indexEq].equipments[items] = val
          }
        })
        indexEq++
      })
    ...
    if (pass) {
      ...
      $.post('http://localhost:3000/insert_queue', Queue, function(failed, textStatus, xhr) {
        if (failed) { // failed
          window.location = "/error"
        } else {
          window.location = "/confirm_create_queue"
        }
      })
    } else {
      $(".alert-trigger").show()
